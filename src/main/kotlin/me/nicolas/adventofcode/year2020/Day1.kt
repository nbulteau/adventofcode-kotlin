...
private fun partTwo(numbers: List<Int>) {
    val set = numbers.flatMap { n1 -> numbers.filter { numbers.contains(2020 - n1 - it) } }.toSet()
    if (set.isNotEmpty()) {
        val calculus = set.reduce { calculus, element -> calculus * element }
        val list = set.toList()
        println("${list[0]} * ${list[1]} * ${list[2]} = $calculus")
    }
}

private fun partOne(numbers: List<Int>) {
    val set = numbers.filter { numbers.contains(2020 - it) }.toSet()
    if (set.isNotEmpty()) {
        val calculus = set.reduce { calculus, element -> calculus * element }
        val list = set.toList()
        println("%d * %d = %d".format(list[0], list[1], calculus))
    }
}
